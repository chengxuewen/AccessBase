...
## rev.2 — dual-Momus absorbed (flows bg_d3e423ff R1-R9 / blockers bg_28b4de08 B1-B9; verdict CHANGES-REQUESTED → this revision)

- **F-A1 (B1 BLOCKER + B2)**: process-lifetime singletons break `mock.results`-based spy isolation in sms-otp.test/ldap-login.test (per-test ctor instances), and onClose-reset NEVER fires in build-once-beforeAll files. Mechanism changed: `utils/managers.ts` exports `resetManagers()` (closes owned singletons duck-typed, clears refs) as the TEST + SHUTDOWN seam; affected test files call `resetManagers()` in `beforeEach` — restores today's exact fresh-per-test semantics; mock.results probes then need NO rewrite (one instance per test, constructed by the first request after reset).
- **F-A2 (R4 + B4, narrative/gate honesty)**: leak mechanism restated — per-request pools churn fresh sockets (idle clients self-close after pg-pool's 10s idleTimeoutMillis, so sequential-exhaustion was overstated) and unbounded CONCURRENT pools each allow max=10 connections → pressure vs max_connections=100 under burst. Live gate re-scoped: measure pg_stat_activity peak under a 24-concurrent-request burst pre-fix vs post-fix (both sides recorded in execution log); drain window honestly noted (flag lives only for hook-chain duration; k8s preStop remains the real load-balancer story).
- **F-A3 (R1)**: shutdownManagers duck-types: `if (typeof (m as {close?:()=>Promise<void>})?.close === 'function')` — plain-object vi.mock factories don't explode afterAll (38-file adaptation avoided).
- **F-A4 (R5)**: getTenantManager is auth.ts-LOCAL closure (auth.ts:94-96) — nothing to import; the holder MOVES into utils/managers.ts as `tenantManagerRef()` and auth.ts delegates.
- **F-A5 (R9)**: NO sessionManager() getter (YAGNI — users.ts/scim.ts already hold singletons; auth.ts:67 has the redis arg).
- **F-A6 (R4 roster)**: replacement set = handler-indentation sites only: auth.ts 17 dynamic-imports + saml.ts:99/197 + oauth.ts:284 + webauthn.ts:99 + setup.ts:242/243/481 + init.ts left as-is (once-per-boot, bounded). Plugin-scope `new X()` (roles.ts:14, users.ts:27-28, tenants.ts:34-36, setup.ts:35, oauth.ts:255, webauthn.ts:62, saml.ts:35-36, auth.ts:67-68, scim.ts:112) UNTOUCHED — roles.test `results[0]` premise preserved.
- **F-C1 (B3 + R2, BLOCKER-adjacent)**: single-session migrate MUST carry pending-determination INSIDE the lock: per file emit `SELECT NOT EXISTS (SELECT 1 FROM schema_migrations WHERE id='X') AS p \gset` + `\if :p BEGIN; \\ir /abs/path; INSERT INTO schema_migrations ...; COMMIT; \endif`. Stamp folded into the SAME locked script, made ATOMIC: `INSERT INTO schema_migrations(id,note) SELECT f,'stamped' FROM unnest(ARRAY[...]) f ON CONFLICT DO NOTHING` gated by a \gset probe of (count=0 AND users-exist); SENTINELS echo stays bash post-session. \ir ABSOLUTE paths mandatory (keeps failed-file stderr signal — ops-migrate broken-SQL test is in the RED roster, plus NEW concurrent-double-run integration test and stamp-inside-lock assertion).
- **F-E1 (R7)**: retention sweeper runs once at boot+60s then every 24h; owns its OWN createDb (closed in the same onClose alongside shutdownManagers); AUDIT_RETENTION_DAYS numeric-validated (non-numeric/0 = disabled, logged); value also seeds from AuditStorage archive config retentionDays if provided (finally a consumer).
- **F-F1 (R3 + B6)**: identity/db gains STRONG `Set<Pool>` registry (add in createDb, delete in closeDb) + exported `getLivePoolStats(): {total,idle,waiting}|null` aggregating totalCount/idleCount/waitingCount (pg-pool@3.14 getters VERIFIED); metrics.ts calls it via OPTIONAL CHAIN (route tests mock the db module without the new export).
- **F-F2 (R3 + B9)**: PG_POOL_MAX parse guard: `Number.isFinite(n) && n>=1 ? n : 10` (negative max = pool never serves = silent hang).
- **F-F3 (B7)**: no stuck latches — degraded gauge computed ON SCRAPE: redis client status (`status==='ready'`?0:1 when configured) + Lockout/FlowToken `lastFallbackAt` decay (60s window) instead of set-true-never-reset.
- **F-F4 (R6 + B5)**: drain flag in NAMED new module `apps/server/src/utils/drain.ts` (setDraining/isDraining); health.ts ready handler checks FIRST (before any DB dial); index shutdown() calls setDraining() before app.close(). No entry-import cycle.
