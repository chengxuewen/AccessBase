...
- **Stamps:** `RoleManager.create` accepts `isSystem?: boolean` (default false); init.ts + setup.ts create the admin role with `isSystem: true`. Self-heal (`permissions-seed.ts` startup) additionally runs an idempotent stamp `UPDATE roles SET is_system = true WHERE name = 'admin'` (all tenants) so existing DBs are protected without manual surgery.
- **Error tags (TENANT_PROTECTED precedent):** identity exports `ROLE_PROTECTED` and `LAST_ADMIN_GUARD` message prefixes; RoleManager throws `ROLE_PROTECTED: ...` on system-role update/delete (replacing bare messages); roles.ts gains a shared error mapper → 409 envelope (mirrors tenants.ts:21).
- **Last-admin guard:** before an operation would leave a tenant with zero ACTIVE users holding the isSystem admin role, throw `LAST_ADMIN_GUARD: ...`. Guard sites (manager-level, single shared predicate helper): `RoleManager.setUserRoles` (when admin role is being removed), `RoleManager.unassignFromUser`, `UserManager.delete`, and user status transitions to suspended (route-level check before suspend). Count predicate: users joined user_roles joined roles WHERE role.isSystem AND users.status='active' AND users.tenantId=:t.
- **API surface:** GET role list/detail expose `isSystem` (types + mapToRole + PaginatedEnvelope rows). Roles.tsx: isSystem rows render edit/delete disabled with a lock affordance; empty-permission confirm never reaches a protected role. 409 tag codes map through the existing apiErrorMessage family (no new i18n machinery — code-keyed message like PERM_001 pattern).
- **Honest limits:** a DBA can still shoot the foot off directly in SQL (unsupported path, recovery doc not in scope); guard counts per tenant — cross-tenant platform rescue uses env-bypass admin creation at init (already exists).

### K3 — CORS production fail-fast

- config.ts: alongside the JWT_SECRET prod throw — `NODE_ENV === 'production' && CORS_ORIGINS empty → throw` with the exact remediation hint (comma-separated allowlist). Dev/test behavior unchanged (reflect). Follow whatever test seam the existing JWT fail-fast check uses (config unit test precedent) so the throw is directly testable.
...
- e2e: existing suites zero new failures; one roles.spec negative (admin row controls disabled).
