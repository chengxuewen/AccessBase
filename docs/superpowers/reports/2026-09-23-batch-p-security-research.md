...
Remaining: **Wave 3** = F12 remainder (RS256 key-PAIR validation vs silent HMAC fallback; NODE_ENV exact-match normalization), F13 (options key whitelist + site.url origin validation), F14 (move tenant-revoke into TenantManager funnel), F15 sweep (/docs prod auth gate, admin bootstrap curl argv password, backup symlink/retention/restore-checksum ordering, Dockerfile frozen-lockfile + pinned tags). Verdict stands **BLOCK** until Wave 3 lands and the docker integration battery runs.
