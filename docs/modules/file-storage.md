...
> **Implementation status (2026-09-23 gap audit):** `design-only (deferred)` — zero code (A8). See [gap-audit](../superpowers/reports/2026-09-23-gap-audit.md).
...
class FilePermissionService {
  async checkAccess(
    ...
    const share = await db
      ...
      .where(and(eq(fileSharesTable.fileId, fileId), eq(fileSharesTable.userId, userId)))
      .limit(1);
