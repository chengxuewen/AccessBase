...
3. 如果是重构 → /remove-ai-slops + /think-before-act
请问具体想做什么？
```
...
## 与 skills-router.md 的关系

- **skills-router.md**（rules）: 始终加载，被动触发（检测到场景时自动推荐）
- **skill-router**（技能）: 主动调用（用户问"怎么做"或意图模糊时）

两者互补：rules 文件处理明确场景，技能处理模糊意图。
