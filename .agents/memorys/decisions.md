...
- **Decision**: All persisted content is English from 2026-09-21: commits, code comments, every markdown document (specs, PLANS, execution records, reports), memory (status/pitfalls/conventions/decisions), skills and rules. Chinese survives only in live AI-agent chat — and there it is REQUIRED (user follow-up same day): agent chat output is always Chinese, while quoted artifacts (code, commands, file contents) keep their original language. NO back-translation of legacy Chinese content — the rule governs new writes; existing files keep their language until rewritten on merit.
