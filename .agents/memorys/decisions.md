...
## D117: Runtime migration sole writer = scripts/migrate.sh (bash+psql over committed drizzle SQL chain) (2026-09-20)

- **Context**: Batch L live audit found both production entrypoints invoked packages/migration's custom CLI which loads TS modules from a non-existent ./migrations dir — zero migrations applied, exit code swallowed by `|| true`: fresh container/deploy databases booted with ZERO tables.
- **Decision**: Runtime migration application in container/deploy entrypoints is done by scripts/migrate.sh — bash + psql applying the committed `packages/migration/drizzle/NNNN_*.sql` chain in lexicographic order, tracked in `schema_migrations(id, applied_at, note)`. Chain dir is an explicit required argument per caller (container image path / repo-root for deploy; out/ never contains the chain). Legacy push-managed DBs: baseline stamp when `users` exists without tracking rows, followed by a chain-head sentinel probe (`SELECT phone FROM users`) that LOUDLY warns (non-fatal) when the volume is behind the chain — schema reconciliation for push-managed DBs remains `db:push`. The custom CLI package stays untouched for its SDD 3-phase contract; only the entrypoints stop calling it. drizzle-kit `up:pg`/`db:migrate` writes `__drizzle_migrations` and is never mixed against a migrate.sh-managed DB.
- **Rationale**: both runtimes ship psql + the chain; drizzle-kit is a devDep whose CLI needs TS-config compilation at runtime; a 60-line bash runner is auditable, loud (`set -euo pipefail`, per-file `-1` transactions, non-zero exit aborts startup), and reuses the generate-step discipline already in conventions. Mis-stamp recoverability is structural: chain files contain zero DROP/TRUNCATE, so a wrongly-unstamped run dies on the first CREATE, no data loss.
- **Reference**: spec docs/superpowers/specs/2026-09-20-batch-l-opsp0-design.md D1; addendum R3/R4/R7/H1; conventions Phase L section.

## D118: Tenant permission partition + binding funnel + platform belt (multi-tenant control plane) (2026-09-20)

- **Context**: Batch G tenant-scoped the data plane, but the control plane was missing — tenants were dead islands (no in-tenant role/user creation path) and the dual-Momus review proved a 3-request privilege-escalation chain: tenant admins hold permissions:read (global permission rows) + roles:write, so ANY role-permission binding path without a partition ceiling lets a tenant admin self-bind tenants:write/options:write and take over the platform.
- **Decision**: (1) The 21 builtin permission names partition exhaustively into TENANT_BINDABLE (9: users*, roles*, permissions:read, audit:read, stats:read) and PLATFORM_ONLY (12: tenants*, options*, clients*, apikeys*, permissions:write/delete), exported from identity services/permission-partition.ts; invariant test (disjoint + union == BUILTIN 21) is the gate for any future permission code addition. (2) The enforcement point is the RoleManager.setRolePermissions funnel: non-default-tenant binding of a non-bindable name throws PERMISSION_NOT_BINDABLE (mapped to 409 by every calling route). Permission rows stay GLOBAL by design; the binding is the ceiling. (3) Tenant cold-start = POST /v1/tenants/:id/bootstrap: find-or-create in-tenant 'admin' role (isSystem, unconditional idempotent stamp), strict-bind the 9 (bindPermissions kernel, never-silent), route-layer user_create password policy, conflict-safe assign replay (200 alreadyBootstrapped). (4) Platform belt: all tenant mutations re-check request.tenantId === DEFAULT_TENANT inside handlers, first, closing the '*'-scope API-key skeleton-key class independent of route gates. (5) API keys are platform-only because such keys bypass every code gate.
- **Rationale**: guard once in the funnel (K convention), not per-route; partition-as-data (invariant test) beats partition-as-scattered-condition; belt-first keeps non-platform callers from probing tenant existence.
- **Reference**: spec docs/superpowers/specs/2026-09-20-batch-lprime-tenant-control-plane-design.md (D1/D1a/D2), REVIEW-ADDENDUM X1/X2/B3/B6; conventions Phase L′ section.

## D119: Metrics exposure posture + backup/restore as secret-bearing ops (2026-09-21)

- **Context**: Batch M added /metrics and pg_dump tooling to an IAM product; dual-Momus caught (a) a generic confirm-on-restore wiping the WRONG database via ambient DATABASE_URL override (BLOCKER), (b) 0644 dump files containing PLAINTEXT sessions.token/oauth tokens (HIGH), and (c) my spec's claim that open-metrics-by-default "mirrors JWT fail-fast philosophy" being factually inverted (JWT hard-fails in prod; metrics was proposed open).
- **Decision**: 1. /metrics stays open when METRICS_TOKEN unset — NO new prod fail-fast (K-T4 brick rule: compose-prod ships without the key; a hard requirement would break shipped paths). Compensations: boot WARN line (warnDegradedChecks, env-only), route-scoped rate-limit opt-out, and Origin-header 404 (no browser reads; @fastify/cors v9 lacks the per-route cors typing — PIT-069). 2. Backups: umask 077 files 0600, PG* env vars never argv, sha256 sidecar; dumps are declared secret material. 3. Restore guard stack: identity echo (user@host:port/db) first, typed database-name confirmation for remote/externally-targeted restores, separate ACCESSBASE_RESTORE_CONFIRM from the reset variable, live-server refusal with explicit --force. 4. compose dev keeps push (loud on failure, retries) rather than switching to migrate.sh — push-managed dev volumes stay the documented convention (L batch baseline stamp bridges promotion).
- **Reference**: spec rev.2 D2/D3/D4 + REVIEW-ADDENDUM B1-B4/R1/R3; PIT-068~070; conventions Phase M section.

## D120: OIDC adapter state = single generic PG table, verbatim payload, parity-over-reasoning (2026-09-21)

- **Context**: Batch N replaces the batch-5 in-memory Map catch-all (restarts wiped every RP refresh token / in-flight consent). Two tempting designs rejected: (a) model-specific tables per kind (Grant/Session/Token…), (b) "clean" semantic consume (DELETE for one-shot credentials).
- **Decision**: ONE table `oidc_adapter_state` (kind,id PK; jsonb payload; derived columns uid(Session-only)/user_code(lower)/grant_id; not_after = sweep hint, NOT a read filter). Payloads round-trip VERBATIM — the provider rehydrates; we never re-map fields (the batch-5 "round-trip drops fields" incident came from our own remapping, not the provider). `consume` = UPDATE jsonb_set marker (memory-adapter parity), `revokeByGrantId` = (kind, grant_id) scoped delete (grantable set parity). Sweeper: 5-min unref interval, errors swallowed, stopSweeper→onClose. Legacy volumes covered by per-file SENTINELS in migrate.sh.
- **Rationale**: the adapter contract is the spec — parity with oidc-provider's own reference memory adapter beats local intuition (PIT-073: DELETE-consume would silently disable the OAuth-BCP replay→revoke-whole-grant defense; the provider reads the marker back). One table keeps TTL/index/sweep logic in exactly one place; derived columns are the only schema coupling.
- **Reference**: spec docs/superpowers/specs/2026-09-21-batch-n-oidc-persistence-design.md rev.3; dual-Momus addendum (B1/B2/B3, $i/$j fiction, v9.12.2 facts); PIT-071~073; conventions Phase N section.

## D121: English-only persistence policy (Chinese restricted to live conversation) (2026-09-21)

- **Context**: The 2026-09-10 split (English for commits/comments/architecture; Chinese for plans, reports and appended memory) produced a two-language corpus: plans/reports/status/pitfalls/conventions in Chinese, specs/decisions/commits in English — grep patterns, cross-references and tooling had to handle both, and appended Chinese entries drifted the memory files away from the repo's English code/documentation baseline.
- **Decision**: All persisted content is English from 2026-09-21: commits, code comments, every markdown document (specs, PLANS, execution records, reports), memory (status/pitfalls/conventions/decisions), skills and rules. Chinese survives only in live AI-agent chat — and there the reply language MIRRORS the user's input language (user refinements same day: Chinese required first, then relaxed to input-mirroring): Chinese input → Chinese output, English input → English output; quoted artifacts (code, commands, file contents) always keep their original language. NO back-translation of legacy Chinese content — the rule governs new writes; existing files keep their language until rewritten on merit.
- **Rationale**: user directive; single persisted language matches the codebase (English identifiers/comments), keeps search/grep/diff and any external contributor consistent, and removes the "which language is this file allowed to be in" decision from every future write.
- **Reference**: conventions.md "Language policy" section (checks included); AGENTS.md CONVENTIONS line.

## D122: CI topology — Gitee origin + Gitee-side one-way mirror to GitHub for Actions (2026-09-23)

- **Context**: Batch L wired full GitHub Actions workflows (ci.yml: lint/typecheck/test/e2e/migrate/build/docker), but the repo's only remote is Gitee — workflows had no execution environment (open item F3). Options were: create a GitHub mirror (zero rewrite, Actions run natively), port to Gitee Go (workflow syntax incompatible, full rewrite), or accept dormant config.
- **Decision**: User confirmed a GitHub mirror repository already exists, synced one-way from Gitee (Gitee-side mirror feature). ci.yml stays as-is and executes on the mirror. No Gitee Go port.
- **Rationale**: zero rewrite of the batch-L CI investment; GitHub Actions ecosystem (postgres:16 services, playwright browsers, codecov-style artifacts) is what the jobs were written against; one-way mirror keeps Gitee as the single write path so no divergence is possible.
- **Consequences**: (1) First push after a workflow change must be verified by a green Actions run on the mirror — until then CI claims stay NOT VERIFIED (recorded in conventions CI-topology bullet). (2) Secrets/env needed by future jobs must be configured on the GitHub side, not Gitee. (3) Any GitHub-side PR/issue workflow is off-policy — everything lands through Gitee origin.
- **Reference**: conventions.md "CI 拓扑" + 2026-09-23 addendum; .github/workflows/ci.yml; batch L status entry (F3 open item closed).

## D123: Retire the openspec skill family (superseded by the superpowers spec pipeline) (2026-09-23)

- **Context**: Audit found all 8 openspec-* skills are dead weight in AccessBase: they orchestrate proposals under `.sisyphus/plans/` (directory never existed; zero use across batches A-N), 5 bodies are MSRCS leftovers (C++/Qt/ROS2 package tables, make.sh commands, project: MSRCS frontmatter), the other 3 require an openspec CLI that is not installed. The project's real, battle-tested proposal pipeline is docs/superpowers/{specs,plans} + dual-Momus review + SDD ledger, already codified in conventions (including the design-doc fact discipline).
- **Decision**: git rm all 8 openspec-* skill directories (history = the archive; revival via git revert + retarget under real usage pressure). Reference sites updated in the same commit: AGENTS.md (counts + example lists), .agents/AGENTS.md structure block, skills-router rule decision-tree edge (architecture-change now routes /think-before-act + /writing-plans), skill-router SKILL.md design row. Archive-to-skills-archive/ rejected as a half measure: git already preserves the bytes and a second copy invites doc-audit churn.
- **Rationale**: YAGNI — retargeting 8 files to serve a second, unused proposal stack duplicates the live pipeline; a deleted-but-revertible commit costs nothing to resurrect if openspec adoption ever becomes real.
- **Reference**: deletion commit in git log (2026-09-23); status.md retirement entry.

## D124: Batch O snapshot strategy — applied chain frozen, v7 rebuilt from the authoritative writer (2026-09-23)

- **Context**: PIT-072's mid-term plan said "upgrade drizzle-kit then regenerate all snapshots." T0 probe falsified the second half: kit 0.31.11's `up` converts the four canonical v5 snapshots but silently skips the hand-written 0004/0005 pair, and `generate` still aborts `data is malformed` on them (the v7 validator requires id/prevId and rejects our legacy index vocabulary). Full-chain regeneration from 0000 was impossible: SQL files 0000-0005 are the applied-runtime contract (schema_migrations tracks them by basename; live DBs carry them), so the chain must stay frozen byte-identical.
- **Decision**: Rebuild only the head region FROM the tool's own output: fresh-generate a full-state v7 snapshot from the current schema in an empty out dir (the authoritative writer form — table-qualified `where`, v7 index vocabulary, policies/isRLSEnabled sections); 0005 := that head; 0004 := head minus the batch-N table (correct historical state before 0005); re-hang a fresh uuid chain from up-converted 0003. Journal untouched at v5 (coexists fine). Schema.ts simultaneously gained the canonical `.where(sql...)` partial-index declarations for the batch-I phone pair (previously schema-absent by PIT-072 necessity) so schema == snapshot forever after.
- **Rationale**: guessing the v7 index shape by hand is how PIT-072 was born; letting 0.31 write the entries eliminates the class. Chain-file immutability keeps migrate.sh/sentinel/ops-migrate contracts byte-stable — verified by live-fire (6/6 fresh, 0/6 idempotent, 17 tables/6 tracked, indexdef WHERE in real pg).
- **Consequences**: `drizzle-kit generate` is usable again (idempotency proven: "No schema changes" zero files); future chain files are tool-produced; the hand-written-trio discipline is retired to a legacy note (Phase O conventions). Residual: 0000-0003 came out of `up` with regenerated ids that differ from the journal-era originals — harmless (nothing consumes snapshot ids except generate's own chain walk, which follows prevId).
- **Reference**: spec T0 VERDICT section (docs/superpowers/specs/2026-09-23-batch-o-drizzle-upgrade-design.md); PIT-072 RESOLVED addendum.

## D125: Rotation replay classifier — 10s concurrent grace distinguishes double-fire from family burn (2026-09-23)

- **Context**: W1-4 made refresh rotation an atomic guarded UPDATE. A 0-row loss conflates two very different events: (a) a benign concurrent double-fire (two tabs, client retry — sibling rotated the token milliseconds ago) and (b) a true replay of a token rotated long ago (theft). Pre-atomic code revoked ALL sessions on any usedAt hit, meaning after the fix every double-fire would mass-logout the user — an availability regression weaponized by anyone who can force concurrent refreshes.
- **Decision**: On guard loss, re-read the row: revoked → 'Session revoked'; usedAt within REPLAY_GRACE_MS (10s) → plain 401 error, NO family burn; usedAt older → replay → revokeAllUserSessions + throw (original batch-A semantics preserved); otherwise expired. Expiry is additionally enforced INSIDE the atomic guard (rev.2 R1: the pre-fix SELECT/UPDATE split let an expired token rotate under concurrency).
- **Rationale**: the grace window preserves the OAuth-BCP reuse-detection posture for the case that matters (replay after loss of control) while refusing to let a race condition become a self-DoS primitive. 10s > observed double-fire skew; false-negative window (attacker replaying within 10s of a legit rotation) requires near-real-time token theft and downgrades to at most one extra 401 for the thief, never to a burn of the victim.
- **Consequences**: the constant is shared truth for SessionManager + unit seeds + real-PG test (session-rotate-race rewinds used_at 60s to exit grace); any tuning updates three places. Concurrent-loser error string carries 'concurrent rotation' for log forensics.
- **Reference**: PIT-077/078; spec rev.2 R1/R2 (docs/superpowers/specs/2026-09-23-p-fix-wave1-auth-core-design.md).

## D126: Gap-audit remediation ladder + documentation status-header gate (2026-09-23)

- **Context**: the five-lane team gap audit (docs/superpowers/reports/2026-09-23-gap-audit.md) found roughly a quarter of module docs describing unwired or phantom behavior (A1-A9: permission propagation, audit hash chain, SSO/trusted-device APIs, email verification, four orphan L0 packages, whole deferred modules without markers). Memory files were honest; design docs were not. Historical fix waves repeatedly suffered from phantom file:line citations (PIT-076 family) — stale docs actively manufacture wasted work.
- **Decision**: (1) remediation ladder Q0 doc-honesty (this commit) -> Q1 promise-fulfillment (UI for shipped backends + S-class code fixes) -> Q2 production foundations (transactions, shared pool, advisory lock, cache coherence, retention executors) -> Q3 IAM completeness (token_version writer, logout coordination, kid rotation, policy engine, device flow, private_key_jwt, SAML IdP) -> Q4 productization (FGA, groups+SCIM Groups, webhooks, templates, portal, helpdesk, Helm); Q3/Q4 gated on explicit product decisions per the architectural-decision gate. (2) Permanent `Implementation status` header on every docs/modules file, enforced by grep gate in conventions. (3) SDD error-code tables frozen; error-codes-reality.md is the generated single source.
- **Rationale**: cheapest anti-lie mechanism is a header line with a grep gate; ordering is impact/effort-ranked with the cheapest wins first (device flow, AuthShell, dark leaks) already placed in Q1.
- **Refs**: gap-audit report §F, D122 (CI mirror), D124 (drizzle v7), PIT-076.
