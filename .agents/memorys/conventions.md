...
- 检查命令 1：`grep -c "resource: '" apps/server/src/routes/permissions-seed.ts` 应 =18（码数变更时同步更新此期望值；2026-09-12 批 C Task 2 15→18：apikeys:read/write/delete + RESOURCES 数组同步加 'apikeys'，缺 RESOURCES 会过滤掉 admin 角色绑定回读）
